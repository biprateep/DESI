...
def comoving_density(redshifts, bins, area, H0=100, Om0=0.3, error=False):
    '''
    Calculate co-moving number density. 
    Inputs
    ------
    redshifts: array of redshifts;
    bins: array of redshift bin edges, with length of (1 + number of bins);
    area: (scalar or array) area in sq. deg. 
    Output
    ------
    densities: array of number densities for each redshift bin;
    densities_err (optional): Poisson error on densities.
    '''

    area_sphere = 41252.96 # Total area of the sky in sq. deg.
    cosmo = FlatLambdaCDM(H0=H0, Om0=Om0)
    densities = np.zeros(len(bins)-1)
    densities_err = np.zeros(len(bins)-1)
    if isinstance(area, (list, tuple, np.ndarray)):
        area = np.array(area)
    else:
        area = area * np.ones(len(bins)-1)
    
    for index in range(len(bins)-1):
        z_low = bins[index]
        z_hi = bins[index+1]
        volume = area[index]/area_sphere*(cosmo.comoving_volume(z_hi) - cosmo.comoving_volume(z_low))
        # convert to Python scalar
        volume = float(volume / (1*u.Mpc)**3)
        counts = np.sum((redshifts>z_low)&(redshifts<z_hi))
        densities[index] = counts/volume
        if error:
            densities_err[index] = np.sqrt(counts)/volume
    if error:
        return densities, densities_err
    else:
        return densities
...
def LRG_cut(sweep, selection="OPT", survey="south"):
    """Implements baseline cuts for LRGs
    Args:
    sweep (astropy.table): Legacy survey sweep file with good quality entries
    selection (str): `OPT` or `IR` which selection cut to apply
    survey (str): `north` or `south`, which survey patch to select
    ...
    if selection == "OPT":
        ...
        if survey == "north":
            ...
            # faint limit
            lrg_opt &= sweep["zfibermag"] < 21.5
            # low-z cut
            mask_red = (sweep["gmag"] - sweep["w1mag"] > 2.67) & (
                sweep["gmag"] - sweep["rmag"] > 1.45
            )
            # ignore low-z cut for faint objects
            mask_red |= (sweep["rmag"] - sweep["w1mag"]) > 1.85
            lrg_opt &= mask_red
            # sliding optical cut
            lrg_opt &= sweep["rmag"] - sweep["zmag"] > (sweep["zmag"] - 16.79) * 0.45
            # low-z sliding optical cut
            lrg_opt &= sweep["rmag"] - sweep["zmag"] > (sweep["zmag"] - 13.76) * 0.19

            return lrg_opt, sweep[lrg_opt]
        ...
        if survey == "south":
            ...
            lrg_opt &= sweep["zfibermag"] < 21.5
            # low-z cut
            mask_red = (sweep["gmag"] - sweep["w1mag"] > 2.6) & (
                sweep["gmag"] - sweep["rmag"] > 1.4
            )
            # ignore low-z cut for faint objects
            mask_red |= (sweep["rmag"] - sweep["w1mag"]) > 1.8
            lrg_opt &= mask_red
            # sliding optical cut
            lrg_opt &= sweep["rmag"] - sweep["zmag"] > (sweep["zmag"] - 16.83) * 0.45
            # low-z sliding optical cut
            lrg_opt &= sweep["rmag"] - sweep["zmag"] > (sweep["zmag"] - 13.80) * 0.19
    ...
    if selection == "IR":
        ...
        if survey == "north":
            ...
            # faint limit
            lrg_ir &= sweep["zfibermag"] < 21.5
            # low-z cut
            lrg_ir &= sweep["rmag"] - sweep["w1mag"] > 1.13
            # sliding IR cut
            lrg_ir &= sweep["rmag"] - sweep["w1mag"] > (sweep["w1mag"] - 17.18) * 1.8
            # low-z sliding IR cut
            lrg_ir &= sweep["rmag"] - sweep["w1mag"] > sweep["w1mag"] - 16.33
        ...
        if survey == "south":
            ...
            lrg_ir &= sweep["zfibermag"] < 21.5
            # low-z cut
            lrg_ir &= sweep["rmag"] - sweep["w1mag"] > 1.1
            # sliding IR cut
            lrg_ir &= sweep["rmag"] - sweep["w1mag"] > (sweep["w1mag"] - 17.22) * 1.8
            # low-z sliding IR cut
            lrg_ir &= sweep["rmag"] - sweep["w1mag"] > sweep["w1mag"] - 16.37
...
def is_red(ztable):
    '''Returns a Boolean for whether a galaxy is red in restframe color, 
        based on redshift and observed colors'''

# check that magnitudes are defined    
    try:
        tmp=len(ztable['gmag'])
    except KeyError:
        ztable['gmag'] = 22.5 - 2.5*np.log10(ztable['FLUX_G']) 
        ztable['rmag'] = 22.5 - 2.5*np.log10(ztable['FLUX_R']) 
        ztable['zmag'] = 22.5 - 2.5*np.log10(ztable['FLUX_Z']) 
 
# trap non-finite values
    isgood1 = np.logical_and(np.isfinite(ztable['gmag']),ztable['rmag'])
    isgood2 = np.logical_and(np.isfinite(ztable['zmag']),ztable['Z'])
    isgood = np.logical_and(isgood1,isgood2)
    
# red/blue cut for z < 0.6    
    isred_lowz = (ztable['gmag']-ztable['rmag']) \
               >  np.minimum(3.1*ztable['Z']+0.31,1.63)
    isred_lowz = np.logical_and(isred_lowz,ztable['Z'] < 0.6)
    
    
# red/blue cut for z > 0.6    
    line1=2.3333*(ztable['Z']-0.5) + 0.94
    line2 = 1.3333*(ztable['Z']-0.8) + 1.6
    line_combo=np.minimum(line1,line2)
    isred_highz =  ( (ztable['rmag']-ztable['zmag']) \
               >  np.minimum(line_combo,1.85) )
    isred_highz = np.logical_and(isred_highz,ztable['Z'] > 0.6)

    isred = np.logical_or(isred_lowz,isred_highz)
    return(np.logical_and(isred,isgood))    
