...
def success_prob(magnitude, exptime, A=-1.1970, B=28.7410):
...
def creates_priority_table(yamlfn):
    ...
    for target in mydict:
        prio_init, prio_fin, ngoal = mydict[target]["PRIORITY_INIT"],mydict[target]["PRIORITY_FIN"], mydict[target]["NGOAL"]
        for nmin in range(ngoal + 1):
            if nmin == ngoal:
                nmax, prio = 99, prio_fin
            else:
