...
def LRG_mod_cut(sweep, selection="OPT", survey="south"):
    ...
    if selection == "OPT":
        ...
        if survey == "north":
            ...
            # low-z cut
            lrg_opt &= sweep["rmag"] - sweep["zmag"] > (sweep["zmag"] - 16.79) * 0.45       # sliding optical cut
            lrg_opt &= sweep["rmag"] - sweep["zmag"] > (sweep["zmag"] - 13.76) * 0.19       # low-z sliding optical cut

            mask_lowz = ((sweep["gmag"]-sweep["rmag"]) > 0.3*(sweep["rmag"]-sweep["w1mag"])+0.9 )  & ( (sweep["gmag"]-sweep["rmag"]) > -1.55*(sweep["rmag"]-sweep["w1mag"])+3.23)
            mask_lowz |= (sweep["rmag"]-sweep["w1mag"] > 1.8)
            lrg_opt &= mask_lowz

            mask_bright = (sweep["gaia_g"]!=0) & (sweep["gaia_g"]<18)
            lrg_opt &= (~mask_bright)

            return lrg_opt, sweep[lrg_opt]

        if survey == "south":
            lrg_opt = sweep["PHOTSYS"]==b"S"
            # LRG_OPT: baseline optical selection
            # non-stellar cut
            lrg_opt &= (
                sweep["zmag"] - sweep["w1mag"]
            ...
            lrg_opt &= sweep["zfibermag"] < 21.5
            # low-z cut
            lrg_opt &= sweep["rmag"] - sweep["zmag"] > (sweep["zmag"] - 16.83) * 0.45       # sliding optical cut
            lrg_opt &= sweep["rmag"] - sweep["zmag"] > (sweep["zmag"] - 13.80) * 0.19       # low-z sliding optical cut

            mask_lowz = ((sweep["gmag"]-sweep["rmag"]) > 0.3*(sweep["rmag"]-sweep["w1mag"])+0.9 )  & ( (sweep["gmag"]-sweep["rmag"]) > -1.55*(sweep["rmag"]-sweep["w1mag"])+3.13)
            mask_lowz |= (sweep["rmag"]-sweep["w1mag"] > 1.8)
            lrg_opt &= mask_lowz
    ...
    if selection == "IR":
        ...
        if survey == "north":
            ...
            #low-z cut
            lrg_ir &= sweep["rmag"] - sweep["w1mag"] > (sweep["w1mag"] - 17.23) * 1.8 # sliding IR cut
            lrg_ir &= sweep["rmag"] - sweep["w1mag"] > (sweep["w1mag"] - 16.33) * 1.  # low-z sliding IR cut
        ...
        if survey == "south":
            ...
            lrg_ir &= sweep["zfibermag"] < 21.5
            #low-z cut
            lrg_ir &= sweep["rmag"] - sweep["w1mag"] > (sweep["w1mag"] - 17.27) * 1.8 # sliding IR cut
            lrg_ir &= sweep["rmag"] - sweep["w1mag"] > (sweep["w1mag"] - 16.37) * 1.  # low-z sliding IR cut
